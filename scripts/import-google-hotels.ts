...
function getShortDescription(place: NormalizedPlace) {
  ...
  if (location.area && location.area !== "Mallorca") {
    return `${name} es un hotel en ${location.area} importado desde Google Places para revision editorial.`;
  }

  return `${name} es un hotel en Mallorca importado desde Google Places para revision editorial.`;
}
...
function buildAiContent(place: NormalizedPlace) {
  ...
  const types = place.types
    ...
    .filter((type) => !["point of interest", "establishment", "lodging"].includes(type))
    .slice(0, 2);
  const specialty = types.length ? types.join(" y ") : "estancias en Mallorca";

  return {
    ai_description: `${name} es un hotel ubicado en ${location}. Su perfil encaja con ${specialty}, dentro de una propuesta pensada para viajeros que buscan una base cuidada en la isla. ${reputationPhrase(place)}`,
    ideal_for: ["escapadas en Mallorca", "viajeros que priorizan ubicacion", "estancias con referencias publicas"],
    what_to_expect: `${name} ofrece una referencia hotelera en ${location}, con datos practicos para valorar ubicacion, reputacion y canales de contacto. Revisa la web o Google Maps para confirmar servicios, disponibilidad y condiciones actuales.`,
    review_summary: `${name} figura como hotel en ${location}. ${reputationPhrase(place)}`,
    faq_auto: [
      {
        question: `Que tipo de hotel es ${name}?`,
        answer: `${name} aparece clasificado como hotel en ${location}, segun los datos publicos importados.`
      },
      {
...
async function main() {
  ...
  if (!existsSync(PREVIEW_PATH)) {
    throw new Error(`Missing preview file: ${PREVIEW_PATH}. Run npm run places:preview:hotels first.`);
  }
  ...
  if (schemaError) {
    throw new Error(`Missing hotel import columns. Apply migrations 002, 004, 005 and 006 before importing. Details: ${schemaError.message}`);
  }
  ...
  for (const place of unique.values()) {
    ...
    const { error } = await supabase.from("businesses").insert({
      ...
      faqs: [],
      seo: {
        title: `${displayName}: hotel en Mallorca | Mallorca Verified`,
  ...
  const { data: topHotels, error: topError } = await supabase
    .from("businesses")
    ...
    JSON.stringify(
      {
        ...
        total_businesses_hotel: count,
        top_10_hotels_by_authority_score: topHotels ?? []
      },
      null,
      2
    )
