...
const BATCHES = [
  { category: "beach", limit: 45 },
  { category: "beach-club", limit: 35 }
];
...
async function main() {
  ...
  const top20Beaches = await topPublished(supabase, "beach", 20);
  const top20BeachClubs = await topPublished(supabase, "beach-club", 20);

  console.log(
    JSON.stringify(
      {
        beaches_publicados: published.beach ?? 0,
        beach_clubs_publicados: published["beach-club"] ?? 0,
        drafts_restantes_por_categoria: draftsRestantesPorCategoria,
        total_published_por_categoria: totalPublishedPorCategoria,
        top_20_beaches_publicados: top20Beaches,
        top_20_beach_clubs_publicados: top20BeachClubs
      },
      null,
      2
    )
