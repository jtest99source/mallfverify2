...
const PREVIEW_PATH = "data/import-previews/beaches-preview.json";
...
function getShortDescription(place: NormalizedPlace) {
  ...
  if (location.area && location.area !== "Mallorca") {
    return `${name} es una playa o cala en ${location.area} importada desde Google Places para revision editorial.`;
  }

  return `${name} es una playa o cala en Mallorca importada desde Google Places para revision editorial.`;
}
...
function reputationPhrase(place: NormalizedPlace) {
  ...
  return `Cuenta con ${place.rating} sobre 5 y ${place.reviews_count} resenas, una base util para valorar su atractivo.`;
}
...
function buildAiContent(place: NormalizedPlace) {
  ...
  const types = place.types
    ...
    .filter((type) => !["point of interest", "establishment", "tourist attraction", "natural feature"].includes(type))
    .slice(0, 2);
  const specialty = types.length ? types.join(" y ") : "paisaje costero y banos en Mallorca";

  return {
    ai_description: `${name} es una playa o cala ubicada en ${location}. Su perfil encaja con ${specialty}, dentro de una propuesta pensada para visitantes que buscan costa, paisaje y una referencia clara en Mallorca. ${reputationPhrase(place)}`,
    ideal_for: ["dias de playa en Mallorca", "viajeros que buscan calas", "planes de costa con referencias publicas"],
    what_to_expect: `${name} ofrece una referencia costera en ${location}, con datos practicos para valorar ubicacion, reputacion y acceso desde Google Maps. Revisa siempre las condiciones locales, el tiempo y el estado del mar antes de ir.`,
    review_summary: `${name} figura como playa o cala en ${location}. ${reputationPhrase(place)}`,
    faq_auto: [
      {
        question: `Que tipo de lugar es ${name}?`,
        answer: `${name} aparece clasificada como playa o cala en ${location}, segun los datos publicos importados.`
      },
      {
...
async function main() {
  ...
  if (!existsSync(PREVIEW_PATH)) {
    throw new Error(`Missing preview file: ${PREVIEW_PATH}. Run npm run places:preview:beaches first.`);
  }
  ...
  if (schemaError) {
    throw new Error(`Missing beach import columns. Apply migrations 002, 004, 005 and 006 before importing. Details: ${schemaError.message}`);
  }
  ...
  for (const place of unique.values()) {
    ...
    const { error } = await supabase.from("businesses").insert({
      ...
      faqs: [],
      seo: {
        title: `${displayName}: playa o cala en Mallorca | Mallorca Verified`,
  ...
  const { data: topBeaches, error: topError } = await supabase
    .from("businesses")
    ...
    JSON.stringify(
      {
        ...
        total_businesses_beach: count,
        top_10_beaches_by_authority_score: topBeaches ?? []
      },
      null,
      2
    )
