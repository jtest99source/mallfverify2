...
const PREVIEW_PATH = "data/import-previews/beach-clubs-preview.json";
...
function getShortDescription(place: NormalizedPlace) {
  ...
  if (location.area && location.area !== "Mallorca") {
    return `${name} es un beach club en ${location.area} importado desde Google Places para revision editorial.`;
  }

  return `${name} es un beach club en Mallorca importado desde Google Places para revision editorial.`;
}
...
function buildAiContent(place: NormalizedPlace) {
  ...
  const types = place.types
    ...
    .filter((type) => !["point of interest", "establishment", "food", "bar", "restaurant"].includes(type))
    .slice(0, 2);
  const specialty = types.length ? types.join(" y ") : "planes junto al mar";

  return {
    ai_description: `${name} es un beach club ubicado en ${location}. Su perfil encaja con ${specialty}, dentro de una propuesta pensada para quienes buscan costa, ambiente y servicio en Mallorca. ${reputationPhrase(place)}`,
    ideal_for: ["planes junto al mar", "dias de playa con servicio", "viajeros que priorizan ambiente y ubicacion"],
    what_to_expect: `${name} ofrece una referencia de beach club en ${location}, con datos practicos para valorar ubicacion, reputacion y canales de contacto. Revisa la web o Google Maps para confirmar servicios, disponibilidad y condiciones actuales.`,
    review_summary: `${name} figura como beach club en ${location}. ${reputationPhrase(place)}`,
    faq_auto: [
      {
        question: `Que tipo de beach club es ${name}?`,
        answer: `${name} aparece clasificado como beach club en ${location}, segun los datos publicos importados.`
      },
      {
        question: `Donde esta ${name}?`,
        answer: `${name} esta situado en ${location}, Mallorca.`
      },
      {
...
async function main() {
  ...
  if (!existsSync(PREVIEW_PATH)) {
    throw new Error(`Missing preview file: ${PREVIEW_PATH}. Run npm run places:preview:beach-clubs first.`);
  }
  ...
  if (schemaError) {
    throw new Error(`Missing beach club import columns. Apply migrations 002, 004, 005 and 006 before importing. Details: ${schemaError.message}`);
  }
  ...
  for (const place of unique.values()) {
    ...
    const { error } = await supabase.from("businesses").insert({
      ...
      tags: place.types,
      best_for: [],
      image: "/images/beach-club.svg",
      gallery: [],
      opening_hours: null,
      faqs: [],
      seo: {
        title: `${displayName}: beach club en Mallorca | Mallorca Verified`,
  ...
  const { data: topBeachClubs, error: topError } = await supabase
    .from("businesses")
    ...
    JSON.stringify(
      {
        ...
        total_businesses_beach_club: count,
        top_10_beach_clubs_by_authority_score: topBeachClubs ?? []
      },
      null,
      2
    )
