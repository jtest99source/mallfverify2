    ...
    <footer className="border-t border-[#F1D3A2] bg-[#10253D] text-white">
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:gap-10 sm:px-6 sm:py-12 md:grid-cols-[1.4fr_1fr] lg:px-8">
        <div className="text-center md:text-left">
          ...
          <p className="mt-3 text-sm leading-7 text-white/70 sm:mt-4">{copy.footer.description}</p>
          <a href={`mailto:${siteConfig.contactEmail}`} className="mt-3 inline-block text-sm text-[#FFD166]/80 hover:text-[#FFD166] sm:mt-5">{siteConfig.contactEmail}</a>
          <p className="mt-3 text-xs text-white/35 sm:mt-6">© {new Date().getFullYear()} Mallorca Verified</p>
        </div>
