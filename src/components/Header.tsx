            ...
            {exploreOpen && (
              <div className="absolute left-0 top-full z-50 mt-2 w-[600px] overflow-hidden rounded-xl border border-[#E7DED0] bg-white shadow-[0_24px_60px_rgba(27,46,75,0.18)]">
                <div className="border-b border-[#E7DED0] bg-[#F8FFFE] px-5 py-3">
                  <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#0E8F72]">18 categorías · Datos verificados</p>
                </div>
                <div className="grid grid-cols-3 gap-px bg-[#F4F0E8] p-px">
                  {Object.entries(categoryConfigs).map(([slug, config]) => (
                    <Link
                      key={slug}
                      href={categoryHref(locale, slug as CategorySlug)}
                ...
                <div className="flex items-center justify-between border-t border-[#E7DED0] bg-[#FFFDF7] px-5 py-3">
                  <Link href={`/${locale}/rankings`} onClick={() => setExploreOpen(false)} className="text-[10px] font-black uppercase tracking-[0.1em] text-[#0E8F72] hover:text-ink">Ver todos los rankings →</Link>
                  <Link href={`/${locale}/guides`} onClick={() => setExploreOpen(false)} className="text-[10px] font-semibold uppercase tracking-[0.08em] text-sage hover:text-ink">Guías</Link>
                </div>
              </div>
            )}
          </div>
      ...
      {mobileOpen && (
        <div className="border-t border-[#E7DED0] bg-[#FFFDF7] px-4 py-4 shadow-xl lg:hidden">
          ...
          <div className="mt-4 grid gap-2 text-[11px] font-black uppercase tracking-[0.1em]">
            ...
            <Link href={`/${locale}/guides`} onClick={() => setMobileOpen(false)} className="rounded-md border border-[#E7DED0] bg-white px-3 py-3">Guías</Link>
            <Link href={methodologyPath(locale)} onClick={() => setMobileOpen(false)} className="rounded-md border border-[#E7DED0] bg-white px-3 py-3">Metodología</Link>
            <Link href={`/${locale}/business`} onClick={() => setMobileOpen(false)} className="rounded-md bg-ink px-3 py-3 text-white">Para negocios</Link>
          </div>
